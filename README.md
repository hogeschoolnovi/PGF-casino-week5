...
## Inleiding
Vorige week heb je blackjack toegevoegd en werkte het casino nog met één actieve speler. Deze week ga je de speleradministratie uitbreiden met profielen voor meerdere spelers.
...
Als de speler nog niet bestaat, vraag je om de geboortedatum, het gender en het startbudget en voeg je het profiel toe aan de `players` dictionary. Laat vervolgens het welkomst bericht zien zoals je die vorige week ook gebruikte, met starbudget, vaste kosten, saldo en de conslusie of de gebruiker genoeg geld heeft meegenomen. 
...
Het saldo wordt gebruikt in het spellen submenu. Nu je dit saldo in het profiel van de actuele gebruiker gaat opslaan, moet je dit ook in het spellen submenu menu aanpassen. Je kunt geen variabele, zoals current_player exporteren, dus je moet twee functies maken. Één functie die je het saldo van de huidige speler terug geeft en één functie waarmee je het saldo van de huidige speler kunt updaten.

In het spellen menu van `main.py` moet je er voor zorgen dat het saldo van de huidige speler na elk gespeeld spel geupdate wordt.
...
Voor het toevoegen moet je opletten dat je met een lege dictionary begint. Als je een spel toevoegt, moet je dus ook eerst die key toevoegen. Als je dit de eerste keer doet, zet je de waarde van die key op 1. Als je dit de tweede (of meer) keer doet, gebruik `+= 1` voor die key. Als je ook de eerste keer `+=` zou gebruiken, krijg je een error.

In het spellen menu in `main.py` moet je er voor zorgen dat het `gespeelde_spellen` attribuut van de huidige speler na elk gespeeld spel wordt geupdate.
...
Er zijn hier twee varianten van. Je kunt een enkel profiel laten zien, maar je kunt ook alle beschikbare profielen laten zien. De eerste optie wordt standaard getoont wanneer de gebruiker het account submenu opent. de tweede wordt pas getoont wanneer de gebruiker de "toon alle profielen" optie kiest in het submenu.
...
Zorg dat er bij het opstarten van de applicatie al 3 of 4 "voor-geïnitialiseerde" profielen in de `players` dict staan, waarmee je al vast kunt inloggen.
...
1. Breid de "account" optie in het hoofd menu uit met een submenu. Schrijf een `show_account_menu()` functie de de opties (Toon alle accounts, Nieuw account, Wissel account, Verwijder account) laat zien. Gebruik `input` om de gebruiker om diens keuze te vragen en maak een if/else of match statement om de juiste functie aan te roepen op basis van de input. Je mag alvast een functie-naam bedenken (en die later maken), maar je mag voor nu ook `pass` gebruiken.
2. De functies gaan we maken in `profiles.py`. Maak dat bestand aan. Zet bovenaan de `players` (dit mag je als een lege dict initialiseren) en zet er `current_player` (dit mag je als None initialiseren).
3. Voordat we de nieuwe functies maken, laten we eerste de oude functies uit main halen. Verplaats `determine_salutation`, `calculate_Age` en `check_age` naar `profiles.py`. Hierbij heb je ook de `MIN_AGE` constante nodig, verplaats die dus ook.
...
5. De `initialize_player(total_cost)` functie ontvang de `TOTAL_COST` constante als argument, omdat we die in main.py definiëren en we die niet opnieuw in profiles.py willen zetten. Nu wordt de profiel initialisatie nog gedaan in de `main` functie, maar al die variabelen (naam, birthdate, gender, startbudget, salutation en balance) zet je nu in de `create_account` functie. De `initialize_player` functie ziet er als volgt uit: 
    ```python
...
8. Als laatste moet je er voor zorgen dat er een profiel gemaakt wordt en dat deze in `players` gezet wordt. Zorg ook dat `current_player` de naam van het zojuist gemaakt account bevat. Een profiel kun je het makkelijkst in een "create_profile(name, birthdate, gender, balance)" functie maken, maar je kunt ook gewoon een dict maken. Zo'n profiel dict ziet er als volgt uit:
    ```python
    {
        "naam": name,
    ...
    ```
9. De profiel dict zet je in de `players` dict onder de key `name`, dus `players[name] = ...`. Vergeet ook niet om `name` ook in `current_player` te zetten.
10. Nu heb je de initizatie en het maken van een nieuw account al geimplementeerd. Je mag in `main.py` dan ook vast de aanroep `create_account(TOTAL_COST)` invullen in het acount submenu bij de optie voor het maken van een nieuw account. Merk op dat je dit zonder een "name" argument doet, dus daar wordt dan de default "None" voor gebruikt.
...
16. Naast het account menu, is er ook in het spellen menu nog wat te updaten. Namelijk, de balance van de huidige spelen moet geupdate worden en de gespeelde spellen van de huidige speler moet geupdate worden.
